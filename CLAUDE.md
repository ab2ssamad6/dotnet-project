...
The **frontend deploys the same way** — `.github/workflows/frontend-image.yml` → `ghcr.io/<owner>/lms-frontend:{latest,sha-<short>}`, run as a third service in the same project (`frontend/README.md` → "Deploy to Railway"). Its Nginx reverse-proxies `/api` to `$API_UPSTREAM` (the API's `*.railway.internal:8080`), keeping the browser same-origin so **CORS is not involved in the deployed stack at all**. `VITE_API_BASE_URL` is deliberately left empty at build time — baking it in would bypass the proxy and reintroduce CORS. Three things in `frontend/` make that image host-agnostic and should not be undone: `nginx.conf` is a template whose `listen ${PORT}` comes from `frontend/docker-entrypoint.sh` (which defaults it to 80, strips a trailing slash off `API_UPSTREAM`, then delegates to the base image's `/docker-entrypoint.sh` for the envsubst pass — hence it is copied in as `/lms-entrypoint.sh` so it cannot shadow that script); the upstream is held in a `set $api_upstream` **variable** with a `resolver` derived from `/etc/resolv.conf`, so nginx resolves per request rather than once at config load (a literal hostname in `proxy_pass` makes the container exit with `host not found in upstream`, because Railway's private DNS is IPv6-only and not up the instant the container boots); and `Host $proxy_host` + `proxy_ssl_server_name` (not `Host $host`) are what let a *public* `https://…up.railway.app` upstream route instead of 404.
